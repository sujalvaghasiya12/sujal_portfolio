...
export default function Hero({ setActiveSection, primaryColor = "#4C8CFF", secondaryColor = "#A855F7", tertiaryColor = "#00E5FF" }: HeroProps) {
  ...
  return (
    <section className="min-h-screen flex items-center justify-center pt-20 relative overflow-hidden text-white" style={{ backgroundColor: "#ffffff" }}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 z-10 text-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease: "easeOut" }}>
          ...
          <motion.div className="flex items-center justify-center gap-3 mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.8 }}>
            <motion.div className="px-4 py-2 rounded-full border backdrop-blur-md shadow-lg" style={{ borderColor: primaryColor + "50", backgroundColor: primaryColor + "10" }} animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 2.5, repeat: Infinity }}>
              <span className="text-sm font-semibold" style={{ color: primaryColor }}>✨ Welcome to my portfolio</span>
            </motion.div>
          </motion.div>

          {/* Name Heading */}
          <motion.h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold mb-6 leading-tight font-display drop-shadow-[0_4px_10px_rgba(0,0,0,0.2)]" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 1 }}>
            <motion.span
          ...
          {/* Subtitle */}
          <motion.p className="text-xl sm:text-2xl font-semibold mb-4" style={{ color: secondaryColor }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.8 }}>
          ...
          <motion.div className="flex flex-col sm:flex-row gap-4 justify-center items-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 0.8 }}>
            <motion.button
              ...
              style={{ background: `linear-gradient(to right, ${primaryColor}, ${secondaryColor})`, color: "#fff", boxShadow: `0 15px 30px ${primaryColor}40` }}
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.08, backgroundColor: primaryColor + "20" }}
              whileTap={{ scale: 0.95 }}
            >
